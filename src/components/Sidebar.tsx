	...
	return (
		<div>
			...
			<aside className={`flex flex-col fixed top-0 left-0 p-2 h-screen w-fit z-40 items-left shadow-lg transition-transform duration-[1000] ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0`}>
				<nav className="flex flex-col p-4 w-full items-left sm:p-2">
					...
					<ul className="space-y-4">
						{menu.map((item) => (
							<li key={item.href} className="menu-mobile relative group flex sm:items-center sm:justify-center">
								<Link href={item.href} className="flex flex-row items-center w-full pl-2 pr-8 py-2 sm:p-2 hover:bg-[var(--color-background)] rounded-xl hover:shadow-xl/20 hover:outline-2 hover:outline-offset-0 hover:outline-[var(--color-brand)]">
